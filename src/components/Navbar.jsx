    ...
    <nav className={`fixed w-full top-0 left-0 z-50 transition-all duration-300
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className={`font-bold text-lg ${dark || scrolled ? 'text-white dark:text-white' : 'text-black'}`}>
            India 360
          </span>
        </Link>
        ...
        <div className="flex items-center gap-3">
          {/* Dark Mode Toggle */}
          <button onClick={toggleDark} className="p-2 rounded-full hover:bg-white/20 dark:hover:bg-gray-700 transition">
            {dark ? <FaSun className="text-yellow-400"/> : <FaMoon className={scrolled ? 'text-white' : 'text-black'} />}
          </button>
          ...
          <button className="md:hidden p-2" onClick={() => setOpen(o => !o)}>
            {open ? <FaTimes className={dark || scrolled ? 'text-white' : 'text-black'} /> : <FaBars className={dark || scrolled ? 'text-white' : 'text-black'} />}
          </button>
        </div>
      </div>

      {/* Mobile Menu */}
      <div className={`md:hidden fixed top-0 left-0 w-full h-screen 
        ...
        <div className="flex gap-4 mt-4">
          <button onClick={() => nav('/login')} className="px-5 py-2 rounded-full border dark:text-white text-black hover:bg-yellow-400 hover:text-black transition">Login</button>
          <button onClick={() => nav('/book')} className="px-5 py-2 rounded-full bg-yellow-400 text-black shadow hover:shadow-lg transition">Book Now</button>
        </div>
      </div>
    </nav>
  )
